...
export async function adminUpdateAppointment(token: string, id: string, data: { status?: Appointment["status"]; stylistId?: string }): Promise<Appointment> {
  return (await request<{ appointment: Appointment }>(`/api/admin/appointments/${id}`, {
